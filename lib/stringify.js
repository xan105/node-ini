...
function stringify (obj, option = {}){
  ...
  for (const name of Object.keys(obj)) 
  {
   ...
   if (isObj(section)) {
      ...
      if(options.comment){
        const comment = atSymbol(comments?.[name], "root");
        if(isArrayOfStringNotEmpty(comment)){
          for (const line of comment) result += line + options.eol;
        }
      }    
      ...
      if(options.comment){
        const comment = atSymbol(comments, "root");
        if(isArrayOfStringNotEmpty(comment)){
          for (const line of comment) result += line + options.eol;
        }
      }   
  ...
  if (options.comment && !hasGlobal && !options.ignoreGlobalSection){
    const comment = atSymbol(comments, "root");
    if (isArrayOfStringNotEmpty(comment)){
      let header = "";
      for (const line of comment) header += line + options.eol;
